...
static uchar LCD_airplane[7];
static uchar scan[4]={0x6f,0x5f,0x3f,0x77};
static uchar Digital_label[12];
static uchar temp_timer_setup[6];  
static uchar page;
void datapage(void);
...
void E_1()
{
PORTD =PORTD & 0xf7;   //E =0
_delay_ms(0.5);
PORTD =PORTD | 0x08;   //E =1
}
void E_0()
{
PORTD =PORTD & 0xf7; //E =0
...
uchar   usbFunctionWrite(uchar *data, uchar len)
{
	...
	switch(*(data+1))
	{
		...
		case 0x04:
			...
			/*DISP_Chr(0x80,Digital_label[*(data+2)]);
			DISP_Chr(0x81,Digital_label[*(data+3)]);
			DISP_Chr(0x82,':');
			DISP_Chr(0x83,Digital_label[*(data+4)]);
			DISP_Chr(0x84,Digital_label[*(data+5)]);
			DISP_Chr(0x85,':');
			DISP_Chr(0x86,Digital_label[*(data+6)]);
			DISP_Chr(0x87,Digital_label[*(data+7)]);
			*/
			temp_timer_setup[0] =Digital_label[*(data+2)];
			temp_timer_setup[1] =Digital_label[*(data+3)];
			temp_timer_setup[2] =Digital_label[*(data+4)];
			temp_timer_setup[3] =Digital_label[*(data+5)];
			temp_timer_setup[4] =Digital_label[*(data+6)];
			temp_timer_setup[5] =Digital_label[*(data+7)];
			page = 1; 
...
int main(void)
{
    ...
    while(1)
	{
		...
		switch(page)
		{
			...
			case 1:
				DISP_Chr(0x80,temp_timer_setup[0]);
				DISP_Chr(0x81,temp_timer_setup[1]);
				DISP_Chr(0x82,':');
				DISP_Chr(0x83,temp_timer_setup[2]);
				DISP_Chr(0x84,temp_timer_setup[3]);
				DISP_Chr(0x85,':');
				DISP_Chr(0x86,temp_timer_setup[4]);
				DISP_Chr(0x87,temp_timer_setup[5]);
			break;
		}
